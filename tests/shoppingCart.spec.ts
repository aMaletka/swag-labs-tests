...
test.describe("Form validity in checkout", () => {
  ...
  test(`When user fill information in checkout without lastname,
   then user receiver message about lack of lastname`, async ({ page }) => {
    const user = getUserData()
    await loginToAccount(page, "standard_user")
    const dashboardPage = new DashboardPage(page)
    await dashboardPage.addFirstProductToCart()
    ...
    await shoppingCartPage.goToCheckout()
    const checkoutInformationPage = new CheckoutInformationPage(page)
    await checkoutInformationPage.fillFormCheckoutStepOne(user.firstname, "", user.postalCode)
    await checkoutInformationPage.clickButtonContinue()
  ...
  test(`When user fill information in checkout without postal code,
   then user receiver message about lack of postal code`, async ({ page }) => {
    const user = getUserData()
    await loginToAccount(page, "standard_user")
    const dashboardPage = new DashboardPage(page)
    await dashboardPage.addFirstProductToCart()
    ...
    await shoppingCartPage.goToCheckout()
    const checkoutInformationPage = new CheckoutInformationPage(page)
    await checkoutInformationPage.fillFormCheckoutStepOne(user.firstname, user.lastname, "")
    await checkoutInformationPage.clickButtonContinue()
